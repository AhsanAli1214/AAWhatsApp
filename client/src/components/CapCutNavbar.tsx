...
export function CapCutNavbar() {
  const [isOpen, setIsOpen] = useState(false);
  const [location] = useLocation();

  return (
    <nav className="fixed top-0 left-0 right-0 z-50 border-b border-border bg-background/95 supports-[backdrop-filter]:bg-background/80 supports-[backdrop-filter]:backdrop-blur-lg">
            ...
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href}>
                <Button
                  variant="ghost"
                  size="sm"
                  className={location === link.href ? "text-primary font-bold" : ""}
                >
                  {link.label}
                </Button>
              </Link>
          ...
          <div className="hidden lg:flex items-center gap-2">
            <DropdownMenu>
              ...
              <DropdownMenuContent align="end">
                ...
                <DropdownMenuItem>
                  <Link href="/capcut-pro" className="flex items-center gap-2 w-full text-primary">
            ...
            <div className="flex flex-col gap-2">
              ...
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href}>
                  <Button
                    variant="ghost"
                    className={`w-full justify-start ${location === link.href ? "text-primary font-bold" : ""}`}
                    onClick={() => setIsOpen(false)}
                  >
                    {link.label}
                  </Button>
